...
class BezierCurve:
    ...
    * `normals` (*type:* `list`, *default:* `None`): Optional input of the normal
    ...
    def __init__(self, pnts, order, tangents=None, normals=None):
        ...
        assert (
            type(pnts) == list and len(pnts) >= 2
        ), "At least two points are needed to calculate the curve"
        ...
        if tangents is not None:
            assert (
                type(tangents) == list and len(tangents) == 2
            ), "Tangent vectors must be provided"
            for t in tangents:
                if type(t) == list:
        ...
        if self._order == 3:
            assert (
                len(self._pnts) == 2
            ), "Two points are needed for the curve to be computed"
            ...
            b = 12 * np.dot(
                self._control_pnts[3] - self._control_pnts[0], tangents[0] + tangents[1]
            )
            ...
            self._control_pnts[1] = (
                self._control_pnts[0] + (1.0 / 3) * alpha * tangents[0]
            )
            self._control_pnts[2] = (
                self._control_pnts[3] - (1.0 / 3) * alpha * tangents[1]
            )
        elif self._order == 4:
            assert (
                len(self._pnts) == 3
            ), "Three points are needed for the curve to be computed"
            ...
            b = 3 * np.dot(
                self._control_pnts[4] - self._control_pnts[0], tangents[0] + tangents[1]
            )
            ...
            if len(self._pnts) == 3:
                ...
                b = 420 * np.dot(
                    self._control_pnts[5] - self._control_pnts[0],
                    tangents[0] + tangents[1],
                )
                c = (
                    -900
                    * np.linalg.norm(self._control_pnts[5] - self._control_pnts[0]) ** 2
                )
                alpha = np.roots([a, b, c]).max()

                # Compute the rest of the control points
                self._control_pnts[1] = (
                    self._control_pnts[0] + 0.2 * alpha * tangents[0]
                )
                self._control_pnts[2] = (
                    2 * self._control_pnts[1] - self._control_pnts[0]
                )
                self._control_pnts[4] = (
                    self._control_pnts[5] - 0.2 * alpha * tangents[1]
                )
                self._control_pnts[3] = (
                    2 * self._control_pnts[4] - self._control_pnts[5]
                )
            elif len(self._pnts) == 2:
                assert tangents is not None and normals is not None
                assert (
                    isinstance(tangents, list) and len(tangents) == 2
                ), "Tangent vectors must be provided"
                assert (
                    isinstance(normals, list) and len(normals) == 2
                ), "Normal vectors must be provided"
                ...
                b = (
                    -28
                    * beta_hat
                    * np.dot((tangents[0] + tangents[1]), normals[1] - normals[0])
                )
                c = (
                    196 * np.linalg.norm(tangents[0] + tangents[1]) ** 2
                    + 120
                    * beta_hat
                    * np.dot(self._pnts[1] - self._pnts[0], normals[1] - normals[0])
                    - 1024
                )
                d = -1680 * np.dot(
                    self._pnts[1] - self._pnts[0], tangents[0] + tangents[1]
                )
                ...
                self._control_pnts[1] = (
                    self._control_pnts[0] + alpha_k / 5.0 * tangents[0]
                )
                self._control_pnts[2] = (
                    2.0 * self._control_pnts[1]
                    - self._control_pnts[0]
                    + beta_hat * alpha_k**2 / 20.0 * normals[0]
                )
                self._control_pnts[4] = (
                    self._control_pnts[5] - alpha_k / 5.0 * tangents[1]
                )
                self._control_pnts[3] = (
                    2.0 * self._control_pnts[4]
                    - self._control_pnts[5]
                    + beta_hat * alpha_k**2 / 20.0 * normals[1]
                )
    ...
    def distance(p1, p2):
        ...
        * `p2` (*type:* list of `float` or `numpy.array`): Point 2
        ...
        assert p1.size == 3 and p2.size == 3, "Both input points must be three elements"
    ...
    def generate_cubic_curve(pnts):
        ...
        * `pnts` (*type:* list of `float` or of `numpy.array`): List of points
        ...
        lengths = [
            BezierCurve.distance(pnts[i + 1], pnts[i]) for i in range(len(pnts) - 1)
        ]
        lengths = [0] + lengths
    ...
    def generate_quintic_curve(pnts):
        ...
        * `pnts` (*type:* list of `float` or of `numpy.array`): List of points
        ...
        alpha_k = lambda k: delta_u(k) / (delta_u(k) + delta_u(k + 1))
        normal_k = lambda k: (
            ((pnts[k + 1] - pnts[k]) / (u[k + 1] - u[k]))
            - ((pnts[k] - pnts[k - 1]) / (u[k] - u[k - 1]))
        ) / (u[k + 1] - u[k - 1])
    ...
    def get_length(self):
        ...
        """
        return self._order * np.linalg.norm(
            self._control_pnts[1] - self._control_pnts[0]
        )
